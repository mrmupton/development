...
$hash_for_user = blowfishEncrypt::hash($_POST['password']);

In a login form:
$is_correct = blowfishEncrypt::check($_POST['password'], $stored_hash_for_user);

In a login form when migrating entries gradually from a legacy SHA-1 hash:
$is_correct = blowfishEncrypt::check(
	$_POST['password'],
	$stored_hash_for_user,
	function($password, $hash) { return $hash == sha1($password); }
);
if ($is_correct && blowfishEncrypt::is_legacy_hash($stored_hash_for_user)) {
	$user->store_new_hash(blowfishEncrypt::hash($_POST['password']));
}
