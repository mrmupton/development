...
var dataGridOutput_pending_charts = {};
var dataGridOutput_check_lib = true;

function dataGridOutput_loadChart(divId)
{
	var swf = findSWF(divId);
	if (swf && typeof swf.load == "function") swf.load(JSON.stringify(dataGridOutput_pending_charts[divId]));
	else setTimeout("dataGridOutput_loadChart('"+divId+"');", 100);
}

function dataGridOutput_get_chart_data(divId) 
{
	setTimeout("dataGridOutput_loadChart('"+divId+"');", 100);
	return JSON.stringify(dataGridOutput_pending_charts[divId]);
}

dataGridOutput.prototype.checkChartLib = function()
{
	dataGridOutput_check_lib = false;
 ...
 * @return
 */
dataGridOutput.prototype.renderBarChart = function(divId, title, labelColumnIndexOrName, options)
{
	with (this) {

		if (dataGridOutput_check_lib && !checkChartLib()) return false;
 ...
 * @return
 */
dataGridOutput.prototype.renderStackedBarChart = function(divId, title, labelColumnIndexOrName, options)
{
	with (this) {
 ...
 * @return
 */
dataGridOutput.prototype.renderPieChart = function(divId, title, valueColumnIndexOrName, labelColumnIndexOrName, options) 
{
	with (this) {
 ...
 * @return
 */
dataGridOutput.prototype.updateChart = function(divId, chart) 
{
	...
	with (this) {
		...
		if (swf && typeof swf.load == "function") swf.load(JSON.stringify(chart));
			...
			var div = _$(divId);
			dataGridOutput_pending_charts[divId] = chart;
			...
			swfobject.embedSWF(this.ofcSwf, 
					...
					"" + (w || 500), 
					"" + (h || 200), 
					"9.0.0", "expressInstall.swf", { "get-data": "dataGridOutput_get_chart_data", "id": divId }, null, 
 ...
 * @return
 */
dataGridOutput.prototype.clearChart = function(divId) 
{
	// how ?
};